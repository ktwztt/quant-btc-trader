...
- `signals.atr` / `signals.mfi` / `signals.cci` / `signals.williamsR` — 各指标值
- `signals.stoch` — `{ k, d }`
- `signals.regime` — `{ regime, score, components }`
- `signals.fundingRate` — 资金费率
- `signals.polymarket` — `{ outcomes: [{price, name}] }`
- `signals.klines` — 最近 500 根 K 线

返回值必须是 `'BUY'`、`'SELL'` 或 `'HOLD'`。

### 配置管理

点击顶栏「设置」按钮，通过标签页管理：
- **API 配置** — 币安 API Key/Secret
- **指标参数** — MA/EMA 周期、RSI/MACD/BB/ATR 等参数
- **策略参数** — 信号权重、决策阈值、RSI 阈值
- **高级** — 刷新间隔、交易对

所有配置保存在浏览器 localStorage，刷新后保留。点击「重置为默认」恢复默认值。
